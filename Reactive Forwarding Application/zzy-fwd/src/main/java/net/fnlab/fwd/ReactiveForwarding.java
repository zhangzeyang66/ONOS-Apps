 ...
 */
//@Component声明一个类为OSGi组件,被@Activate和@deactivate标记的方法分别在组件被激活和关闭时被调用
//当@Component的immediate属性为true时,表明组件载入时就立即激活它
@Service
@Component(immediate = true)
public class ReactiveForwarding implements ForwardingMapService {
    ...
    //把应用中需要用到的服务都写入成员变量,并使用@Reference注解声明对这些服务的引用
    //这样在应用加载时Karaf会用ONOS中已经存在的服务实例初始化这些引用(Interface),在应用中就可以使用这些服务了
    ...
    //下面是这个Component的属性,可以用于Component Configuration
    ...
    public void activate(ComponentContext context) {
        ...
        //向packetService添加对数据分组的处理器
        packetService.addProcessor(processor, PacketProcessor.director(2));
        topologyService.addListener(topologyListener);
        //本类函数
        readComponentConfiguration(context);
        //对交换机进行设置,使其将匹配失败的包发送到ONOS控制器进行处理
        requestIntercepts();
    ...
    public void deactivate() {
        ...
        //命令交换机不再将相关类型的包发送至ONOS控制器处理
        withdrawIntercepts();
        //删除该应用下发的所有流表项
        flowRuleService.removeFlowRulesById(appId);
        //撤销包处理器processor,使其不再进行包处理
        packetService.removeProcessor(processor);
        topologyService.removeListener(topologyListener);
        //删除对processor的引用,使它可以被垃圾回收器回收
        processor = null;
        log.info("Stopped");
    }
     ...
     */
    //对交换机进行设置,使其将匹配失败的包发送到ONOS控制器进行处理(通过下发流表项)
...
//                        pkt.receivedFrom(), ethPkt.getSourceMAC(), ethPkt.getDestinationMAC());
            ...
            endPoints.put(HostId.hostId(ethPkt.getSourceMAC()), id);
    ...
    private void installRule(PacketContext context, PortNumber portNumber) {
        ...
        // If PacketOutOnly or ARP packet then forward directly to output port
